    ...
    if request.method == 'POST':
        try:
            ...
            if(password == cnfpassword):
                db.session.add(User(fname=request.form['fname'], lname=request.form['lname'],
                               email=request.form['email'], password=generate_password_hash(password, method='sha256')))
                db.session.commit()
    ...
    if request.method == 'GET':
        ...
        if (data and check_password_hash(data.password, password)) and recaptcha.verify():
            msg = Message('Confirm Email',
                          sender='devnpatel18@gnu.ac.in', recipients=[email])
